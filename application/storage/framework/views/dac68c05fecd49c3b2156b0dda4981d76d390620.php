...
<script type="text/javascript">
    ...
    function buildDropzone()
    {
        var myDropzone = new Dropzone("#uploadform",
                {
                    url: "<?php echo e(config('app')['mediaconverter.public.web.url']); ?>/assets/store",
        ...
        myDropzone.on("complete", function(file) {
            ...
            $.get("<?php echo e(config('app')['mediaconverter.public.web.url']); ?>/assets/process?file="+file.name+'&time'+Date.now(), function( data ) {
    ...
    function pingInDesignserver()
    {
        ...
        $.get("<?php echo e(config('app')['mediaconverter.public.web.url']); ?>/indesignserver/ping", function( data ) {
    ...
    {
        var webSocket = WS.connect("<?php echo e(config('app')['mediaconverter.public.websocket.url']); ?>");
        ...
        {
            var message = "Disconnected for " + error.reason + " with code " + error.code + " Url: <?php echo e(config('app')['mediaconverter.public.websocket.url']); ?>" ;
        ...
        webSocket.on("socket/connect", function (session)
        {
            ...
            session.subscribe("mittax/mediaconverter/topic/converter/ticketcreated", function (uri, payload)
            {
                ...
                else
                {
                    ...
                    $('#message').prepend('<tr><td><img id="' + message.ticketId +'" class="rounded float-left" src="<?php echo e(asset('images/loading_blue.gif')); ?>" alt="Loading" title="Loading" width="120" /></td><td><strong>Folder: </strong>' + message.uuid + '</small><br/><div id="error_' + message.ticketId+'"></div></td><td>' + message.version +'</td><td>' + message.extension +'</td></tr>');
                }
            });
